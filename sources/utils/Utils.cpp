...
std::pair<int, std::string> parsePortAndPassword(char **av) {
    ...
    if (port < 1024 || port > 65535) {
        throw std::invalid_argument("Port number is out of range of available ports: '" + std::string(av[1]) + "'");
    }

    // validate password
    std::string password = av[2];
    if (password.find(" ") != std::string::npos || (password.length() < 8 || password.length() > 16)) {
        throw std::invalid_argument("Password must be 8-16 characters long and contain no spaces.");
    }
